...
class Order(models.Model):
    ...
    paymentCode = models.CharField(max_length=10, default='', blank=True)
    ...
    def save(self, *args, **kwargs):
        if not self.paymentCode:
            self.paymentCode = uuid.uuid4().hex[:8].upper()
        super().save(*args, **kwargs)

    def __str__(self):
        return f"Order {self.paymentCode} - {self.createdAt}"
...
class ShippingAddress(models.Model):
    ...
    guestName = models.CharField(max_length=200, null=True, blank=True)
    guestEmail = models.EmailField(null=True, blank=True)
    guestPhone = models.CharField(max_length=50, null=True, blank=True)
    _id = models.AutoField(primary_key=True, editable=False)
